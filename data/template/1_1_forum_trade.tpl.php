<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('trade');
0
|| checktplrefresh('./template/default/forum/trade.htm', './template/default/common/header.htm', 1364212652, '1', './data/template/1_1_forum_trade.tpl.php', './template/default', 'forum/trade')
|| checktplrefresh('./template/default/forum/trade.htm', './template/default/common/footer.htm', 1364212652, '1', './data/template/1_1_forum_trade.tpl.php', './template/default', 'forum/trade')
|| checktplrefresh('./template/default/forum/trade.htm', './template/default/common/header_common.htm', 1364212652, '1', './data/template/1_1_forum_trade.tpl.php', './template/default', 'forum/trade')
|| checktplrefresh('./template/default/forum/trade.htm', './template/default/common/pubsearchform.htm', 1364212652, '1', './data/template/1_1_forum_trade.tpl.php', './template/default', 'forum/trade')
;?>
...
<div class="z"><a href="./" class="nvhm" title="首页"><?php echo $_G['setting']['bbname'];?></a> <em>&rsaquo;</em> 确认购买信息</div>
</div>
<div id="ct" class="ct2 wp cl">
<h1 class="mt">1.确认商品</h1>
<div class="mn">
<script src="<?php echo $_G['setting']['jspath'];?>forum_viewthread.js?<?php echo VERHASH;?>" type="text/javascript"></script>
<script type="text/javascript">
zoomstatus = parseInt(<?php echo $_G['setting']['zoomstatus'];?>);
var feevalue = 0;
<?php if($trade['price'] > 0) { ?>var price = <?php echo $trade['price'];?>;<?php } if($_G['setting']['creditstransextra']['5'] != -1 && $trade['credit']) { ?>var credit = <?php echo $trade['credit'];?>;var currentcredit = <?php echo getuserprofile('extcredits'.$_G['setting']['creditstransextra']['5']); ?>;<?php } ?>
</script>

<form method="post" autocomplete="off" id="tradepost" name="tradepost" action="forum.php?mod=trade&amp;action=trade&amp;tid=<?php echo $_G['tid'];?>&amp;pid=<?php echo $pid;?>">
<input type="hidden" name="formhash" value="<?php echo FORMHASH;?>" />
<div class="bm">
<h3 class="bm_h">确认购买信息</h3>
<div class="torder bm_c cl">
<div class="spvimg">
<?php if($trade['aid']) { ?>
<a href="forum.php?mod=viewthread&amp;do=tradeinfo&amp;tid=<?php echo $trade['tid'];?>&amp;pid=<?php echo $trade['pid'];?>"><img src="<?php echo getforumimg($trade['aid']); ?>" width="90" /></a>
<?php } else { ?>
<a href="forum.php?mod=viewthread&amp;do=tradeinfo&amp;tid=<?php echo $trade['tid'];?>&amp;pid=<?php echo $trade['pid'];?>"><img src="<?php echo IMGDIR;?>/nophotosmall.gif" /></a>
<?php } ?>
</div>
<div class="spi cl">
<dl>
...
&nbsp;<?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['5']]['title'];?>&nbsp;<strong><?php echo $trade['credit'];?></strong>&nbsp;<?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['5']]['unit'];?>
<?php } ?>
</dd>
<?php if($trade['locus']) { ?>
<dt>所在地点</dt><dd><?php echo $trade['locus'];?></dd><?php } ?>
<dt>卖家</dt><dd><a href="home.php?mod=space&amp;uid=<?php echo $trade['sellerid'];?>" target="_blank"><?php echo $trade['seller'];?></a></dd>
</dl>
</div>
<table summary="确认购买信息" cellspacing="0" cellpadding="0" class="tfm">
<tr>
<th>支付总额</th>
<td>
<?php if($trade['price'] > 0) { ?><strong id="caculate"></strong>&nbsp;元&nbsp;&nbsp;<?php } if($_G['setting']['creditstransextra']['5'] != -1 && $trade['credit']) { ?><?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['5']]['title'];?>&nbsp;<strong id="caculatecredit"></strong>&nbsp;<?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['5']]['unit'];?>&nbsp;<span id="crediterror"></span><?php } ?>
</td>
</tr>
<tr>
...
<td>
<p>
<?php if($trade['transport'] == 1) { ?><input type="hidden" name="transport" value="1">卖家承担运费<?php } if($trade['transport'] == 2) { ?><input type="hidden" name="transport" value="2">买家承担运费<?php } if($trade['transport'] == 3) { ?><input type="hidden" name="transport" value="3">虚拟物品<?php } if($trade['transport'] == 4) { ?><input type="hidden" name="transport" value="4">买家收到货物后直接支付给物流公司<?php } ?>
</p>
<?php if($trade['transport'] == 1 or $trade['transport'] == 2 or $trade['transport'] == 4) { if(!empty($trade['ordinaryfee'])) { ?><label class="lb"><input class="pr" type="radio" name="fee" value="1" checked="checked" <?php if($trade['transport'] == 2) { ?>onclick="feevalue = <?php echo $trade['ordinaryfee'];?>;calcsum()"<?php } ?> />平邮 <?php echo $trade['ordinaryfee'];?> 元</label><?php if($trade['transport'] == 2) { ?><script type="text/javascript">feevalue = <?php echo $trade['ordinaryfee'];?></script><?php } } if(!empty($trade['expressfee'])) { ?><label class="lb"><input class="pr" type="radio" name="fee" value="3" checked="checked" <?php if($trade['transport'] == 2) { ?>onclick="feevalue = <?php echo $trade['expressfee'];?>;calcsum()"<?php } ?> />快递 <?php echo $trade['expressfee'];?> 元</label><?php if($trade['transport'] == 2) { ?><script type="text/javascript">feevalue = <?php echo $trade['expressfee'];?></script><?php } } if(!empty($trade['emsfee'])) { ?><label class="lb"><input class="pr" type="radio" name="fee" value="2" checked="checked" <?php if($trade['transport'] == 2) { ?>onclick="feevalue = <?php echo $trade['emsfee'];?>;calcsum()"<?php } ?> /> EMS <?php echo $trade['emsfee'];?> 元</label><?php if($trade['transport'] == 2) { ?><script type="text/javascript">feevalue = <?php echo $trade['emsfee'];?></script><?php } } } ?>
</td>
</tr>
<tr>
<th>交易方式</th>
<td>
<?php if(!$_G['uid']) { ?>
<label><input type="hidden" name="offline" value="0" checked="checked" />在线交易</label>
<?php } elseif(!$trade['account'] && !$trade['tenpayaccount']) { ?>
<input type="hidden" name="offline" value="1" checked="checked" />担保交易
<?php } else { ?>
<label class="lb"><input type="radio" class="pr" name="offline" value="0" checked="checked" />在线交易</label>
<label class="lb"><input type="radio" class="pr" name="offline" value="1" />担保交易</label>
<?php } ?>
</td>
</tr>
...
<td><input type="text" id="buyername" name="buyername" maxlength="50" value="<?php echo $lastbuyerinfo['buyername'];?>" class="pt" /></td>
</tr>
<tr>
<th><label for="buyercontact">收货地址</label></th>
<td><input type="text" id="buyercontact" name="buyercontact" maxlength="100" size="40" value="<?php echo $lastbuyerinfo['buyercontact'];?>" class="pt" /></td>
</tr>
<tr>
<th><label for="buyerzip">收货人邮编</label></th>
<td><input type="text" id="buyerzip" name="buyerzip" maxlength="10" value="<?php echo $lastbuyerinfo['buyerzip'];?>" class="pt" /></td>
</tr>
<tr>
<th><label for="buyerphone">收货人电话</label></th>
<td><input type="text" id="buyerphone" name="buyerphone" maxlength="20" value="<?php echo $lastbuyerinfo['buyerphone'];?>" class="pt" /></td>
</tr>
<tr>
<th><label for="buyermobile">收货人手机</label></th>
<td><input type="text" id="buyermobile" name="buyermobile" maxlength="20" value="<?php echo $lastbuyerinfo['buyermobile'];?>" class="pt" /></td>
</tr>
<?php } else { ?>
...
<?php } ?>
<tr>
<th valign="top"><label for="buyermsg">备注信息</label></th>
<td>
<textarea id="buyermsg" name="buyermsg" rows="5" class="pt"></textarea>
<div class="xg2">200 字以内</div>
</td>
</tr>
<tr>
<th>&nbsp;</th>
<td class="pns">
<button class="pn pnc" type="submit" id="tradesubmit" name="tradesubmit" value="true"><span>确认购买</span></button>
<?php if(!$_G['uid']) { ?><em class="xg2">您目前为游客，购买后您无法在本版块查看交易状态，请到支付网站查询</em><?php } ?>
</td>
</tr>
</table>
</div>
...
<script type="text/javascript">
function calcsum() {
<?php if($trade['price'] > 0) { ?>$('caculate').innerHTML = (price * $('tradepost').number.value + feevalue);<?php } if($_G['setting']['creditstransextra']['5'] != -1 && $trade['credit']) { ?>
v = (credit * $('tradepost').number.value + feevalue);
if(v > currentcredit) {
$('crediterror').innerHTML = '<font color="red">您的积分不足</font>';
