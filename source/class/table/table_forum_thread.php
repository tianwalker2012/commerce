...
class table_forum_thread extends discuz_table
{
	...
	public function fetch_all_first_by_authorid($uids,$start,$limit){
		if($uids){
			   $uids=implode(',', $uids); 
			$sql="select p.tid,p.pid,p.authorid,p.subject,p.message,p.dateline,t.author,p.tid,p.attachment,l.mapx longitude,l.mapy latitude";
			...
			echo $sql.="order by t.dateline desc  limit $start,$limit ";
