...
class table_forum_forum extends discuz_table
{
	...
	public function fetch_all_grouptype($start,$limit) {
		...
		$sql.="FROM ".DB::table($this->_table)." f inner JOIN ".DB::table('forum_forumfield')." ff  on f.fid=ff.fid WHERE f.status='3' AND ( f.fid ='37' or f.fup=37 ) ";
	...
	public function fetch_all_groupmytype($grouplist,$start,$limit) {
		$sql ="SELECT f.fid,f.name,ff.description,ff.founderuid,ff.foundername,ff.rank, ff.category,ff.dateline,ff.membernum,ff.foundername,ff.description,ff.activity   ";
		// $sql.=" , ( select count(*) from pre_forum_thread  )   dynamic_count ";
		$sql.="FROM ".DB::table($this->_table)." f inner JOIN ".DB::table('forum_forumfield')." ff  on f.fid=ff.fid WHERE f.status='3' AND f.fid in ( $grouplist ) ";
	     $sql.="ORDER BY   ff.dateline desc limit $start,$limit ";
		return DB::fetch_all($sql);
	}
	...
	{
  	return DB::fetch_all("SELECT fid from pre_forum_forum where name like '%".$infor."%' ");
	}
	/**/
